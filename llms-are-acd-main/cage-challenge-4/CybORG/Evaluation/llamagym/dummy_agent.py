...
class ReactRemoveBlueAgent(LinearAgent):
    ...
    def __init__(
        self, name: str, print_action_output: bool = False, print_obs_output: bool = False):
        super().__init__(name, print_action_output, print_obs_output)
        self._setup_subnets()
        self.end_episode()

    def get_message(self, observation: dict, message_space: Space) -> np.ndarray:
    ...
    def set_initial_values(self, action_space, observation):
        self.reverse_ip_map = {v["Interface"][0]["ip_address"]: k for k, v in observation.items() if k not in ("success", "action")}
    ...
    def get_action(self, observation, action_space):
        ...
        if sel is not None:
            if issubclass(action_class, ControlTraffic):
    ...
    def _get_proc_info(self, obs: dict):
        ...
        for k, v in obs.items():
            ...
            attackers |= {self.reverse_ip_map[x["remote_address"]] for x in malicious_conns if x["remote_address"] in malicious_conns}
